...
class GitHubMirrorClient < MirrorClient
  @@mirror_error_map = {
    "Octokit::Unauthorized": MirrorClientUnauthorized,
    "Octokit::Forbidden": MirrorClientForbidden,
    "Octokit::NotFound": MirrorClientNotFound,
    "Octokit::UnprocessableEntity": MirrorClientGenericError,
    "Octokit::ServerError": MirrorClientServerError
  }
  ...
  def error_for_github_error(github_error)
    ...
    return MirrorClientGenericError unless @@mirror_error_map[error_name]
  ...
  def commit(repo, sha, client_name: '', expires: nil)
    ...
    branches = client.commit_branches(repo, sha, accept: preview_header)
                     .map { |b| { name: b.name, protection: b.protected } }
    Commit.new(sha, data.commit.author.date, branches: branches)
  rescue *@@github_errors => e
    raise error_for_github_error(e), format(
